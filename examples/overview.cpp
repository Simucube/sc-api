...
int main(int argc, char* argv[]) {
    ...
    // We could use sc_api::Api directly also
    sc_api::Api api;
    ...
    sc_api::NoAuthControlEnabler control_enabler(
        &api, sc_api::Session::ControlFlag::control_telemetry | sc_api::Session::control_sim_data, "sc-api-example",
        user_info);
    ...
    while (true) {
        if (auto event = api_event_queue->tryPopFor(std::chrono::milliseconds(200))) {
            if (auto session_state_changed = std::get_if<sc_api::event::SessionStateChanged>(&*event)) {
                ...
                if (session_state == sc_api::SessionState::connected_control) {
                    ...
                    namespace p = sc_api::sim_data::vehicle;
