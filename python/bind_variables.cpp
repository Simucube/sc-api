...
static nb::object read_array(const VariableDefinition& var) {
    ...
    switch (var.type.getBaseType()) {
        ...
        case Type::boolean: {
            ...
            sc_api::RevisionCountedArrayRef<uint8_t> ref(var.type.getArraySize(), var.value_ptr);
...
void bind_variables(nb::module_& m) {
    ...
    nb::class_<Type>(m, "Type",
        ...
        .def_prop_ro("base_type", &Type::getBaseType, "The scalar element type (BaseType enum) of this variable.")
        ...
        .def_prop_ro("is_bit", &Type::isBit, "True if this variable represents a single bit within a larger integer.")
        ...
        .def_prop_ro("is_invalid", &Type::isInvalid, "True if this type descriptor is invalid or unset.")
        .def_prop_ro("is_base_type", &Type::isBaseType,
                     "True if this type describes a simple scalar (not a bit-field and not an array).")
        .def(
            "__eq__", [](const Type& a, const Type& b) { return a == b; }, nb::is_operator())
        .def("__repr__", [](const Type& self) { return "<Type " + self.toString() + ">"; });
    ...
    nb::class_<VariableDefinition>(m, "VariableDefinition",
        ...
        .def_prop_ro(
            "name", [](const VariableDefinition& self) { return std::string(self.name); },
            "Unique name of the variable.")
        .def_prop_ro(
            "type", [](const VariableDefinition& self) { return self.type; }, "Data type descriptor for this variable.")
        .def_prop_ro(
            "device_session_id", [](const VariableDefinition& self) { return self.device_session_id; },
            "Session-scoped identifier of the device that owns this variable.")
        .def_prop_ro(
            "flags", [](const VariableDefinition& self) { return self.flags; },
            "Variable flags providing additional metadata about this variable.")
        .def("__bool__", [](const VariableDefinition& self) { return static_cast<bool>(self); })
        .def_prop_ro(
            "value",
            [](const VariableDefinition& self) -> nb::object {
                if (!self.value_ptr) return nb::none();
                if (self.type.isArray()) return read_array(self);
                return read_value(self);
            },
            "Current value read from shared memory. Returns a scalar (bool, int, float, str), "
            "a numpy ndarray for array types, or None if unavailable.")
        .def("__repr__", [](const VariableDefinition& self) {
            return "<VariableDefinition name='" + std::string(self.name) + "' type=" + self.type.toString() +
    ...
    nb::class_<VariableDefinitions>(m, "VariableDefinitions",
        ...
        .def(
            ...
            [](const VariableDefinitions& self) {
                return nb::make_iterator(nb::type<VariableDefinitions>(), "VariableDefinitionsIterator", self.begin(),
        ...
        .def(
            ...
            nb::arg("index"), nb::keep_alive<0, 1>(), "Return the variable definition at the given index.")
        .def(
            "find",
            [](const VariableDefinitions& self, const std::string& name,
               std::optional<DeviceSessionId> device) -> nb::object {
                auto def = device ? self.find(name, *device) : self.find(name);
                if (!def) return nb::none();
                return nb::cast(def);
            },
            nb::arg("name"), nb::arg("device") = nb::none(), nb::keep_alive<0, 1>(),
