...
FfbPipeline::~FfbPipeline() {
    if (isActive()) {
        sc_api::CommandRequest req("ffb", "free_pipeline");
        req.docAddElement("device_session_id", device_.id);
        req.docAddElement("pipeline_id", pipeline_id_);
        action_builder_.getSession()->asyncCommand(std::move(req), [](const sc_api::AsyncCommandResult&) {});
    }
}
...
bool FfbPipeline::configure(const PipelineConfig& config) {
    ...
    sc_api::CommandResult result = action_builder_.getSession()->blockingCommand(std::move(req));
    if (result.isSuccess()) {
        sc_api::util::BsonReader reader(result.getPayload().data(), result.getPayload().size());
...
bool FfbPipeline::generateEffect(Clock::time_point start_timestamp, Clock::duration sample_time, const float* samples,
    ...
    if (!sc_api::buildEffectOffsetDataAction(action_builder_, ref, start_timestamp, sample_time, samples,
                                             sample_count)) {
        return false;
    }
...
bool FfbPipeline::stop() {
    ...
    EffectPipelineRef ref = {device_.id, (uint8_t)pipeline_id_};
    sc_api::buildEffectClearAction(action_builder_, ref);
    return action_builder_.sendNonBlocking() == ActionResult::complete;
}
