...
std::shared_ptr<sim_data::SimData> detail::SimDataProvider::parseSimData() {
...
std::string_view Vehicle::getName() const { return getOrDefault(sc_api::sim_data::vehicle::name, ""); }

Track::Track(std::string_view id, const uint8_t* raw_bson) : SimDataSubSectionGetters(raw_bson), id_(id) {}

std::string_view Track::getName() const { return getOrDefault(sc_api::sim_data::track::name, ""); }
